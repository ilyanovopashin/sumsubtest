...
SUMSUB_SECRET_KEY = "K067SSc6pShDY5kqcrOE7CNootEUNp2e"  # Example: Hej2ch71kG2kTd1iIUDZFNsO5C1lh5Gq
SUMSUB_APP_TOKEN = "prd:NYENKI1rcllTMnRDNiDn5xbR.e9Ic3EqxJzHzFhxy3D17KfNIjVkfUsAR"  # Example: sbx:uY0CgwELmgUAEyl4hNWxLngb.0WSeQeiYny4WEqmAALEAiK2qTC96fBad
...
def add_document(applicant_id):
    ...
    with open('img.jpg', 'wb') as handle:
        response = requests.get('https://drive.google.com/uc?export=download&id=127TWCSWZ-moh7udQqwZ7IkXypk4lvElO', stream=True)
        if not response.ok:
        ...
        for block in response.iter_content(1024):
            if not block:
                break
            handle.write(block)
    payload = {"metadata": '{"idDocType":"SELFIE", "country":"RUS"}'}
    resp = sign_request(
        requests.Request('POST', SUMSUB_TEST_BASE_URL + '/resources/applicants/' + applicant_id + '/info/idDoc',
                         data=payload,
                         files=[('content', open('C:/Users/isnov/1/img.jpg', 'rb'))]
                         ))
    sw = requests.Session()
...
def main():
    ...
    #external_user_id = 'test_ext_ID'
    level_name = 'basic-kyc-level'
    applicant_id = '62a32cdcd989e10001d06924'
